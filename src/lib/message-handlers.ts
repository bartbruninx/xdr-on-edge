...
// ============================================================================
// Alerts Message Handler
// ============================================================================

export async function handleAlertsMessage(
  message: SecurityMessage, 
  oauth: OAuthService, 
  api: MicrosoftSecurityApiClient,
  logAuditEvent: (level: AuditLogEntry['level'], category: AuditLogEntry['category'], action: string, details: Record<string, any>) => void
  ...
  try {
    ...
    const alerts = await api.getAlerts(accessToken, filters);
    
    logAuditEvent('info', 'api', 'alerts_retrieved', {
      count: alerts.value?.length || 0,
      hasFilters: !!filters
    });
    
    return createSuccessResponse(alerts, message.requestId);
    ...
    if (error instanceof Error && 'code' in error) {
      ...
      // Handle network/timeout errors specifically
      if (authError.code === 'NETWORK_ERROR') {
        logAuditEvent('warn', 'api', 'alerts_network_timeout', {
          error: authError.message,
          originalError: authError.details?.originalError
        });
        return createErrorResponse('NETWORK_ERROR', 'Request timeout - please try again', message.requestId);
      ...
      // Handle other API errors
      if (authError.code === 'API_ERROR') {
        logAuditEvent('error', 'api', 'alerts_api_error', {
          error: authError.message,
          status: authError.details?.status,
          code: authError.code
        });
    ...
    logAuditEvent('error', 'api', 'alerts_error', {
      error: error instanceof Error ? error.message : 'Unknown error',
      errorName: error instanceof Error ? error.name : 'Unknown',
      stack: error instanceof Error ? error.stack : undefined
    });
    ...
    return createErrorResponse(
      'API_ERROR', 
      error instanceof Error ? error.message : 'Unknown error occurred while fetching alerts', 
      message.requestId
    );
  }
}
...
// ============================================================================
// Advanced Hunting Message Handler
// ============================================================================

export async function handleHuntMessage(
  message: SecurityMessage, 
  oauth: OAuthService, 
  api: MicrosoftSecurityApiClient,
  logAuditEvent: (level: AuditLogEntry['level'], category: AuditLogEntry['category'], action: string, details: Record<string, any>) => void
  ...
  try {
    const accessToken = await oauth.getValidAccessToken();
    const query = message.data as any;
    
    const results = await api.runAdvancedHuntingQuery(accessToken, query);
    
    logAuditEvent('info', 'api', 'hunting_query_executed', {
      queryLength: query.query?.length || 0,
      resultCount: results.results?.length || 0,
      timespan: query.timespan
    });
    
    return createSuccessResponse(results, message.requestId);
    ...
    if (error instanceof Error && 'code' in error) {
      ...
      // Handle network/timeout errors specifically
      if (authError.code === 'NETWORK_ERROR') {
        logAuditEvent('warn', 'api', 'hunting_network_timeout', {
          error: authError.message,
          originalError: authError.details?.originalError
        });
        return createErrorResponse('NETWORK_ERROR', 'Request timeout - please try again', message.requestId);
      ...
      // Handle other API errors
      if (authError.code === 'API_ERROR') {
        logAuditEvent('error', 'api', 'hunting_api_error', {
          error: authError.message,
          status: authError.details?.status,
          code: authError.code
        });
    ...
    logAuditEvent('error', 'api', 'hunting_query_error', {
      error: error instanceof Error ? error.message : 'Unknown error',
      errorName: error instanceof Error ? error.name : 'Unknown',
      stack: error instanceof Error ? error.stack : undefined,
      queryLength: (message.data as any).query?.length || 0
    });
    
    // Return a proper error response instead of throwing
    return createErrorResponse(
      'API_ERROR', 
      error instanceof Error ? error.message : 'Unknown error occurred while executing hunting query', 
      message.requestId
    );
  }
}
...
// ============================================================================
// Update Alert Message Handler
// ============================================================================

export async function handleUpdateAlertMessage(
  message: SecurityMessage, 
  oauth: OAuthService, 
  api: MicrosoftSecurityApiClient,
  logAuditEvent: (level: AuditLogEntry['level'], category: AuditLogEntry['category'], action: string, details: Record<string, any>) => void
  ...
  try {
    const accessToken = await oauth.getValidAccessToken();
    const { alertId, update } = message.data as any;
    
    const updatedAlert = await api.updateAlert(accessToken, alertId, update);
    
    logAuditEvent('info', 'api', 'alert_updated', {
      alertId,
      updateFields: Object.keys(update)
    });
    
    return createSuccessResponse(updatedAlert, message.requestId);
    ...
    if (error instanceof Error && 'code' in error) {
      ...
      // Handle network/timeout errors specifically
      if (authError.code === 'NETWORK_ERROR') {
        logAuditEvent('warn', 'api', 'alert_update_network_timeout', {
          error: authError.message,
          originalError: authError.details?.originalError
        });
        return createErrorResponse('NETWORK_ERROR', 'Request timeout - please try again', message.requestId);
      ...
      // Handle other API errors
      if (authError.code === 'API_ERROR') {
        logAuditEvent('error', 'api', 'alert_update_api_error', {
          error: authError.message,
          status: authError.details?.status,
          code: authError.code
        });
    ...
    logAuditEvent('error', 'api', 'alert_update_error', {
      alertId: (message.data as any).alertId,
      error: error instanceof Error ? error.message : 'Unknown error',
      errorName: error instanceof Error ? error.name : 'Unknown',
      stack: error instanceof Error ? error.stack : undefined
    });
    ...
    return createErrorResponse(
      'API_ERROR', 
      error instanceof Error ? error.message : 'Unknown error occurred while updating alert', 
      message.requestId
    );
  }
}
